...
import {
  URUGUAI_VENUES,
  URUGUAI_PERMIT_LABEL,
  type UruguaiVenueItem,
  type UruguaiPermit,
} from "@/lib/data-uruguai-locacoes";
...
function VenueCard({ venue }: { venue: UruguaiVenueItem }) {
  const visual = CATEGORY_VISUAL[venue.category];
  const CategoryIcon = visual.Icon;
  const permit = URUGUAI_PERMIT_LABEL[venue.permit as UruguaiPermit];
...
type CidadeFilter = "all" | UruguaiVenueItem["cidade"];
...
const CIDADE_TABS: { value: CidadeFilter; label: string; periodo: string }[] = [
  ...
  { value: "cancun",      label: "Cancún",      periodo: "10–13 jun" },
  { value: "miami-1",     label: "Miami 1ª",    periodo: "13–16 jun" },
  { value: "boston",      label: "Boston",      periodo: "16–20 jun" },
  { value: "miami-2",     label: "Miami 2ª",    periodo: "20–24 jun" },
  { value: "guadalajara", label: "Guadalajara", periodo: "24–28 jun" },
];
...
export function LocacoesUruguai() {
  ...
  const filtered = useMemo(() => {
    return URUGUAI_VENUES.filter((v) => {
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          nas cidades do roteiro da Equipe Uruguai. Filtre por cidade, categoria, período do dia e permit.
        </p>
      </div>
            ...
            {CIDADE_TABS.map((tab) => {
              ...
              return (
                ...
                >
                  {tab.label}
                  {tab.periodo && (
                    <span className="font-mono text-[10px] opacity-70">{tab.periodo}</span>
                  )}
                </button>
              );
            })}
      ...
      <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        <ul className="ml-4 list-disc space-y-1 text-xs text-amber-900/90 dark:text-amber-200/90">
          <li>
            <strong>Cancún — espaços públicos:</strong> SEMA Quintana Roo (sema.qroo.gob.mx). Zona Arqueológica El Rey: autorização prévia do INAH (inah.gob.mx) obrigatória para filmagem comercial.
          </li>
          <li>
            <strong>Miami — espaços públicos:</strong> Miami-Dade Film Office (filmiami.org) para Bayfront Park e Downtown. Miami Beach Film &amp; Print Office para South Pointe Park e Miami Beach.
          </li>
          <li>
            <strong>Boston:</strong> Mayor's Office of Special Events (boston.gov) para Boston Common &amp; Public Garden. Prazo recomendado: 5–10 dias úteis.
          </li>
          <li>
            <strong>Guadalajara — espaços públicos:</strong> SIOP Jalisco (siop.jalisco.gob.mx) ou Secretaria de Cultura do Município de Guadalajara.
          </li>
          <li>
            <strong>Instituto Cultural Cabañas (UNESCO):</strong> Autorização institucional direta — contato via direccion@hospiciocabanas.org.mx. Exige justificativa e contrato específico.
          </li>
          <li>
            <strong>Privado (qualquer cidade):</strong> Negociar diretamente com o estabelecimento — Nomads Hotel, Sugar/EAST Miami, 1 Hotel South Beach e Hotel Demetria têm tarifas específicas para uso comercial.
          </li>
        </ul>
      </div>
    </main>
