...
import {
  FRANCA_VENUES,
  FRANCA_PERMIT_LABEL,
  type FrancaVenueItem,
  type FrancaPermit,
} from "@/lib/data-franca-locacoes";
...
function VenueCard({ venue }: { venue: FrancaVenueItem }) {
  const visual = CATEGORY_VISUAL[venue.category];
  const CategoryIcon = visual.Icon;
  const permit = FRANCA_PERMIT_LABEL[venue.permit as FrancaPermit];
...
type CidadeFilter = "all" | FrancaVenueItem["cidade"];
...
const CIDADE_TABS: { value: CidadeFilter; label: string; periodo: string }[] = [
  ...
  { value: "los-angeles",      label: "Los Angeles",       periodo: "09–13 jun" },
  { value: "boston-seaport-1", label: "Boston Seaport I",  periodo: "13–14 jun" },
  { value: "new-jersey",       label: "New Jersey",        periodo: "14–17 jun" },
  { value: "boston-midtown",   label: "Boston Midtown",    periodo: "16–20 jun" },
  { value: "philadelphia",     label: "Philadelphia",      periodo: "20–23 jun" },
  { value: "boston-seaport-2", label: "Boston Seaport II", periodo: "23–28 jun" },
];
...
export function LocacoesFranca() {
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          nas cidades do roteiro da Equipe França. Filtre por cidade, categoria, período do dia e permit.
        </p>
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        <ul className="ml-4 list-disc space-y-1 text-xs text-amber-900/90 dark:text-amber-200/90">
          <li>
            <strong>Los Angeles:</strong> FilmLA (filmla.com) — gratuito para espaços públicos, prazo 3–5 dias úteis.
          </li>
          <li>
            <strong>Boston:</strong> Boston Film Bureau (boston.gov/departments/arts-and-culture) — prazo 5–10 dias úteis.
          </li>
          <li>
            <strong>New Jersey / NYC públicos:</strong> NYC MOME (nyc.gov/film) para espaços de Nova York; Hoboken e Weehawken têm processos municipais próprios.
          </li>
          <li>
            <strong>Philadelphia:</strong> Philadelphia Film Office (visitphilly.com) — prazo 5–10 dias úteis, exige seguro de responsabilidade.
          </li>
          <li>
            <strong>Privado (qualquer cidade):</strong> Negociar diretamente com o estabelecimento — rooftops de hotel, Fenway Park, Prudential e Franklin Institute cobram taxas específicas para uso comercial.
          </li>
        </ul>
      </div>
    </main>
