...
import {
  ESPANHA_VENUES,
  ESPANHA_PERMIT_LABEL,
  type EspanhaVenueItem,
  type EspanhaPermit,
} from "@/lib/data-espanha-locacoes";
...
function VenueCard({ venue }: { venue: EspanhaVenueItem }) {
  const visual = CATEGORY_VISUAL[venue.category];
  const CategoryIcon = visual.Icon;
  const permit = ESPANHA_PERMIT_LABEL[venue.permit as EspanhaPermit];
...
type CidadeFilter = "all" | EspanhaVenueItem["cidade"];
...
const CIDADE_TABS: { value: CidadeFilter; label: string; periodo: string }[] = [
  ...
  { value: "chattanooga",  label: "Chattanooga",  periodo: "10–13 jun" },
  { value: "atlanta",      label: "Atlanta",      periodo: "13–25 jun" },
  { value: "guadalajara",  label: "Guadalajara",  periodo: "25–28 jun" },
];
...
export function LocacoesEspanha() {
  ...
  const filtered = useMemo(() => {
    return ESPANHA_VENUES.filter((v) => {
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          nas cidades do roteiro da Equipe Espanha. Filtre por cidade, categoria, período do dia e permit.
        </p>
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        <ul className="ml-4 list-disc space-y-1 text-xs text-amber-900/90 dark:text-amber-200/90">
          <li>
            <strong>Chattanooga:</strong> City of Chattanooga (chattanoogatn.gov) para espaços públicos. Lookout Mountain Point Park exige autorização do National Park Service (nps.gov/chch).
          </li>
          <li>
            <strong>Atlanta:</strong> Atlanta Film Office (atlantaga.gov) — gratuito para espaços públicos como Piedmont Park. Prazo 5–10 dias úteis.
          </li>
          <li>
            <strong>Guadalajara — espaços públicos:</strong> SIOP Jalisco (siop.jalisco.gob.mx) ou Secretaria de Cultura do Município de Guadalajara. Prazo variável.
          </li>
          <li>
          ...
          <li>
            <strong>Privado (qualquer cidade):</strong> Negociar diretamente — Edwin Hotel, Hotel Demetria, Ponce City Market e Georgia Aquarium têm taxas específicas para uso comercial.
          </li>
        </ul>
      </div>
    </main>
