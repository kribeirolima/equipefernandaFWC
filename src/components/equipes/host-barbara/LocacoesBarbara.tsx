...
import {
  BARBARA_VENUES,
  BARBARA_LOC_PERMIT_LABEL,
  type BarbaraVenueItem,
  type BarbaraLocPermit,
} from "@/lib/data-barbara-locacoes";
...
function VenueCard({ venue }: { venue: BarbaraVenueItem }) {
  const visual = CATEGORY_VISUAL[venue.category];
  const CategoryIcon = visual.Icon;
  const permit = BARBARA_LOC_PERMIT_LABEL[venue.permit as BarbaraLocPermit];
  return (
...
type CidadeFilter = "all" | BarbaraVenueItem["cidade"];
...
const CIDADE_TABS: { value: CidadeFilter; label: string; periodo: string }[] = [
  ...
  { value: "toronto",      label: "Toronto ON",         periodo: "09–13 jun" },
  { value: "atlanta",      label: "Atlanta GA",         periodo: "13–15 · 20–21 jun" },
  { value: "kansas-city",  label: "Kansas City MO",     periodo: "15–17 jun" },
  { value: "boston",       label: "Boston MA",          periodo: "17–20 jun" },
  { value: "dallas",       label: "Dallas TX",          periodo: "21–22 · 27–29 jun" },
  { value: "houston",      label: "Houston TX",         periodo: "22–24 jun" },
  { value: "guadalajara",  label: "Guadalajara Jal.",   periodo: "24–27 jun" },
];
...
export function LocacoesBarbara() {
  ...
  const filtered = useMemo(() => {
    return BARBARA_VENUES.filter((v) => {
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          nas 7 cidades do roteiro da Equipe Host · Bárbara Coelho. Filtre por cidade, categoria, período e permit.
        </p>
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        <ul className="ml-4 list-disc space-y-1 text-xs text-amber-900/90 dark:text-amber-200/90">
          <li><strong>Toronto — espaços públicos:</strong> City of Toronto Film Office (toronto.ca/film) para Nathan Phillips Square e parques municipais. Prazo recomendado de 3–5 dias úteis.</li>
          <li><strong>Atlanta — espaços públicos:</strong> Georgia World Congress Authority (gwcc.com) para Centennial Olympic Park.</li>
          <li><strong>Kansas City — espaços públicos:</strong> Kansas City Film Office (filmkc.com).</li>
          <li><strong>Boston — espaços públicos:</strong> Mayor's Office of Arts &amp; Culture / Boston Film Bureau (boston.gov/film). Prazo recomendado de 5 dias úteis.</li>
          <li><strong>Dallas — espaços públicos:</strong> Dallas Film Commission (filmdallas.com) para Klyde Warren Park e outros espaços municipais.</li>
          <li><strong>Houston — espaços públicos:</strong> Houston Film Commission (filmhouston.org) para Discovery Green e parques municipais.</li>
          <li><strong>Guadalajara — espaços públicos:</strong> SIOP Jalisco e Secretaria de Cultura Municipal para parques. Prazo mínimo de 10 dias úteis.</li>
          <li><strong>Privado (qualquer cidade):</strong> Negociar diretamente — CN Tower, Thompson Rooftop, Reunion Tower, Envoy Hotel, Prudential Skywalk, Glenn Hotel, POST Houston e Casa Fayette têm tarifas específicas para uso comercial.</li>
        </ul>
      </div>
    </main>
  );
