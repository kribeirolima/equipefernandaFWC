...
import {
  ARGENTINA_VENUES,
  ARGENTINA_LOC_PERMIT_LABEL,
  type ArgentinaVenueItem,
  type ArgentinaLocPermit,
} from "@/lib/data-argentina-locacoes";
...
function VenueCard({ venue }: { venue: ArgentinaVenueItem }) {
  const visual = CATEGORY_VISUAL[venue.category];
  const CategoryIcon = visual.Icon;
  const permit = ARGENTINA_LOC_PERMIT_LABEL[venue.permit as ArgentinaLocPermit];
  return (
...
type CidadeFilter = "all" | ArgentinaVenueItem["cidade"];
...
const CIDADE_TABS: { value: CidadeFilter; label: string; periodo: string }[] = [
  ...
  { value: "kansas-city", label: "Kansas City MO",  periodo: "10–21 jun" },
  { value: "dallas",      label: "Dallas TX",       periodo: "21 jun–01 jul" },
];
...
export function LocacoesArgentina() {
  ...
  const filtered = useMemo(() => {
    return ARGENTINA_VENUES.filter((v) => {
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          nas 2 cidades do roteiro da Equipe Argentina · Rafa Morientes e Djalminha. Filtre por cidade, categoria, período e permit.
        </p>
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        <ul className="ml-4 list-disc space-y-1 text-xs text-amber-900/90 dark:text-amber-200/90">
          <li><strong>Kansas City — espaços públicos:</strong> Kansas City Film Office (filmkc.com) para Liberty Memorial, Power & Light District e Crossroads Arts District. Solicitar com pelo menos 5 dias úteis de antecedência.</li>
          <li><strong>Dallas — espaços públicos:</strong> Dallas Film Commission (filmdallas.com) para Klyde Warren Park e The Rustic (área pública adjacente). Prazo recomendado de 3–5 dias úteis.</li>
          <li><strong>Privado (qualquer cidade):</strong> Negociar diretamente — 1 Hotel KC Rooftop, Reunion Tower GeO-Deck, Monarch/Virgin Hotels e Fogo de Chão/Texas de Brazil têm tarifas específicas para uso comercial.</li>
        </ul>
      </div>
    </main>
  );
