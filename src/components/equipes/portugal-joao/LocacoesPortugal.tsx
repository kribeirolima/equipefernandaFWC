...
import {
  PORTUGAL_VENUES,
  PORTUGAL_LOC_PERMIT_LABEL,
  type PortugalVenueItem,
  type PortugalLocPermit,
} from "@/lib/data-portugal-locacoes";
...
function VenueCard({ venue }: { venue: PortugalVenueItem }) {
  const visual = CATEGORY_VISUAL[venue.category];
  const CategoryIcon = visual.Icon;
  const permit = PORTUGAL_LOC_PERMIT_LABEL[venue.permit as PortugalLocPermit];
  return (
...
type CidadeFilter = "all" | PortugalVenueItem["cidade"];
...
const CIDADE_TABS: { value: CidadeFilter; label: string; periodo: string }[] = [
  ...
  { value: "cdmx",    label: "Cidade do México", periodo: "08–12 jun" },
  { value: "miami",   label: "Miami FL",          periodo: "12 jun–01 jul" },
  { value: "houston", label: "Houston TX",        periodo: "15–17 · 21–23 jun" },
];
...
export function LocacoesPortugal() {
  ...
  const filtered = useMemo(() => {
    return PORTUGAL_VENUES.filter((v) => {
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          nas 3 cidades do roteiro da Equipe Portugal · João Barretto. Filtre por cidade, categoria, período e permit.
        </p>
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        <ul className="ml-4 list-disc space-y-1 text-xs text-amber-900/90 dark:text-amber-200/90">
          <li><strong>Cidade do México — espaços públicos:</strong> Comisión de Filmaciones de la CDMX (filmaciones.cdmx.gob.mx) — Bosque de Chapultepec. Prazo recomendado: 5–7 dias úteis.</li>
          <li><strong>Miami — espaços públicos:</strong> Miami-Dade Film & Entertainment (filmiami.org) — Bayfront Park, The Wharf. Prazo: 3–5 dias úteis.</li>
          <li><strong>Houston — espaços públicos:</strong> Houston Film Commission (houstonfilmcommission.com) — POST Houston, Discovery Green, Pitch 25. Prazo: 3–5 dias úteis.</li>
          <li><strong>Privado (qualquer cidade):</strong> Negociar diretamente — Torre Latinoamericana, Balcón del Zócalo, Hard Rock Cafe CDMX, Sugar at East Miami, Frost Science Museum, Fogo de Chão têm tarifas específicas para uso editorial.</li>
        </ul>
      </div>
    </main>
  );
