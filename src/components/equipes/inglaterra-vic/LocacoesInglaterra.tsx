...
import {
  INGLATERRA_VENUES,
  INGLATERRA_LOC_PERMIT_LABEL,
  type InglaterraVenueItem,
  type InglaterraLocPermit,
} from "@/lib/data-inglaterra-locacoes";
...
function VenueCard({ venue }: { venue: InglaterraVenueItem }) {
  const visual = CATEGORY_VISUAL[venue.category];
  const CategoryIcon = visual.Icon;
  const permit = INGLATERRA_LOC_PERMIT_LABEL[venue.permit as InglaterraLocPermit];
  return (
...
type CidadeFilter = "all" | InglaterraVenueItem["cidade"];
...
const CIDADE_TABS: { value: CidadeFilter; label: string; periodo: string }[] = [
  ...
  { value: "dallas",       label: "Dallas TX",       periodo: "11–18 jun" },
  { value: "houston",      label: "Houston TX",      periodo: "18–20 jun" },
  { value: "new-jersey",   label: "New Jersey",      periodo: "20–24 · 26–28 jun" },
  { value: "kansas-city",  label: "Kansas City MO",  periodo: "24–26 jun" },
];
...
export function LocacoesInglaterra() {
  ...
  const filtered = useMemo(() => {
    return INGLATERRA_VENUES.filter((v) => {
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          nas 4 cidades do roteiro da Equipe Inglaterra · Vic Leite. Filtre por cidade, categoria, período e permit.
        </p>
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        <ul className="ml-4 list-disc space-y-1 text-xs text-amber-900/90 dark:text-amber-200/90">
          <li><strong>Dallas — espaços públicos:</strong> Dallas Film Commission (filmdallas.com) — Klyde Warren Park, The Rustic. Prazo recomendado: 3–5 dias úteis.</li>
          <li><strong>Houston — espaços públicos:</strong> Houston Film Commission (houstonfilmcommission.com) — POST Houston Skylawn, Discovery Green, Pitch 25. Prazo: 3–5 dias úteis.</li>
          <li><strong>New Jersey — espaços públicos:</strong> NJ Motion Picture & Television Commission (njfilm.org) — Liberty State Park, Sinatra Park. Prazo: 5 dias úteis.</li>
          <li><strong>Kansas City — espaços públicos:</strong> Kansas City Film Office (filmkc.com) — Liberty Memorial, Crossroads, Power & Light District. Prazo: 5 dias úteis.</li>
          <li><strong>Privado (qualquer cidade):</strong> Negociar diretamente — Reunion Tower, Monarch/Virgin Hotels, Texas de Brazil, Top of the Rock, 230 Fifth, Fogo de Chão, 1 Hotel KC têm tarifas específicas para uso editorial.</li>
        </ul>
      </div>
    </main>
  );
