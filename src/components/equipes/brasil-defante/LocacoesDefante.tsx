...
import {
  DEFANTE_VENUES,
  DEFANTE_PERMIT_LABEL,
  type DefanteVenueItem,
  type DefantePermit,
} from "@/lib/data-defante-locacoes";
...
function VenueCard({ venue }: { venue: DefanteVenueItem }) {
  const visual = CATEGORY_VISUAL[venue.category];
  const CategoryIcon = visual.Icon;
  const permit = DEFANTE_PERMIT_LABEL[venue.permit as DefantePermit];
  return (
...
type CidadeFilter = "all" | DefanteVenueItem["cidade"];
...
const CIDADE_TABS: { value: CidadeFilter; label: string; periodo: string }[] = [
  ...
  { value: "nova-york",     label: "Nova York NY",    periodo: "02–06 · 12–14 · 19–22 jun" },
  { value: "cdmx",          label: "Cidade do México",periodo: "06–12 jun" },
  { value: "atlanta",       label: "Atlanta GA",      periodo: "14–16 jun" },
  { value: "kansas-city",   label: "Kansas City MO",  periodo: "15–17 jun" },
  { value: "philadelphia",  label: "Philadelphia PA", periodo: "17–19 · 22–23 jun" },
  { value: "miami",         label: "Miami FL",        periodo: "23–28 jun" },
];
...
export function LocacoesDefante() {
  ...
  const filtered = useMemo(() => {
    return DEFANTE_VENUES.filter((v) => {
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          nas 6 cidades do roteiro da Equipe Brasil · Defante. Filtre por cidade, categoria, período do dia e permit.
        </p>
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        <ul className="ml-4 list-disc space-y-1 text-xs text-amber-900/90 dark:text-amber-200/90">
          <li><strong>Nova York — espaços públicos:</strong> NYC Mayor's Office of Media &amp; Entertainment (film.nyc.gov) para Central Park e outros espaços públicos. Formulário online — prazo recomendado de 5–10 dias úteis.</li>
          <li><strong>Cidade do México — espaços públicos:</strong> Secretaría de Cultura CDMX (cultural.cdmx.gob.mx) para Bosque de Chapultepec e parques municipais. Prazo mínimo de 10 dias úteis.</li>
          <li><strong>Atlanta — espaços públicos:</strong> Georgia World Congress Authority (gwcc.com) para Centennial Olympic Park. Mayor's Office of Film &amp; Entertainment para outros espaços.</li>
          <li><strong>Kansas City — espaços públicos:</strong> Kansas City Film Office (filmkc.com) para parques e espaços públicos municipais.</li>
          <li><strong>Philadelphia — espaços públicos:</strong> Philadelphia Film Office (film.org) para Love Park / JFK Plaza. Gratuito — prazo recomendado de 5–10 dias úteis.</li>
          <li><strong>Miami — espaços públicos:</strong> Miami-Dade Film Office (filmiami.org) para Margaret Pace Park e espaços públicos do município.</li>
          <li><strong>Privado (qualquer cidade):</strong> Negociar diretamente com o local — 230 Fifth, Glenn Hotel, Assembly Rooftop, The Broken Shaker, Terraza Cha Cha Chá e observatórios têm tarifas específicas para uso comercial.</li>
        </ul>
      </div>
    </main>
  );
