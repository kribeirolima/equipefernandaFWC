...
import {
  ALEMANHA_VENUES,
  ALEMANHA_PERMIT_LABEL,
  type AlemanhaVenueItem,
  type AlemanhaPermit,
} from "@/lib/data-alemanha-locacoes";
...
function VenueCard({ venue }: { venue: AlemanhaVenueItem }) {
  const visual = CATEGORY_VISUAL[venue.category];
  const CategoryIcon = visual.Icon;
  const permit = ALEMANHA_PERMIT_LABEL[venue.permit as AlemanhaPermit];
...
type CidadeFilter = "all" | AlemanhaVenueItem["cidade"];
...
const CIDADE_TABS: { value: CidadeFilter; label: string; periodo: string }[] = [
  ...
  { value: "winston-salem", label: "Winston-Salem",  periodo: "09–12 jun" },
  { value: "houston",       label: "Houston",        periodo: "12–15 jun" },
  { value: "toronto",       label: "Toronto",        periodo: "15–21 jun" },
  { value: "hoboken",       label: "Hoboken / NYC",  periodo: "21–26 jun" },
];
...
export function LocacoesAlemanha() {
  ...
  const filtered = useMemo(() => {
    return ALEMANHA_VENUES.filter((v) => {
    ...
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          nas cidades do roteiro da Equipe Alemanha. Filtre por cidade, categoria, período do dia e permit.
        </p>
      </div>
      ...
      <div className="space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        <ul className="ml-4 list-disc space-y-1 text-xs text-amber-900/90 dark:text-amber-200/90">
          <li>
            <strong>Winston-Salem:</strong> Contato com a Prefeitura de Winston-Salem (winstonsalem.com). Prazo 5–10 dias úteis.
          </li>
          <li>
            <strong>Houston:</strong> Houston Film Commission (houstonfilmcommission.com) — gratuito, prazo 3–5 dias úteis.
          </li>
          <li>
            <strong>Toronto:</strong> Toronto Film Office (toronto.ca/film) — prazo 5–10 dias úteis, exige seguro de responsabilidade.
          </li>
          <li>
            <strong>Hoboken / NYC público:</strong> NYC MOME (nyc.gov/film) para NYC; Hoboken/Weehawken têm processos municipais próprios.
          </li>
          <li>
            <strong>Privado (qualquer cidade):</strong> Negociar diretamente com o estabelecimento — rooftops de hotel, CN Tower, Space Center cobram taxas específicas para uso comercial.
          </li>
        </ul>
      </div>
    </main>
