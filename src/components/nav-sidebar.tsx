...
import { PlaneTakeoff, MapPin, Camera, Calendar, Plane, Users } from "lucide-react";
...
export function NavSidebar({ open }: { open: boolean }) {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside
        className={`fixed top-14 left-0 bottom-0 z-40 hidden lg:flex flex-col bg-white border-r border-gray-200 transition-[width] duration-200 overflow-hidden overflow-y-auto ${
          open ? "w-60" : "w-16"
        }`}
      >
        <nav className="flex-1 p-2 space-y-0.5">
          ...
          {EQUIPES.length > 0 && (
            <>
              ...
              {EQUIPES.map((equipe) => {
                const href = `/equipes/${equipe.slug}`;
                const active = pathname === href;
                return (
                  <Link
                    key={equipe.slug}
                    href={href}
                    title={!open ? equipe.name : undefined}
